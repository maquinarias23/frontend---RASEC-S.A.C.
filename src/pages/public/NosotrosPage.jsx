...
import { HiOutlineChevronLeft, HiOutlineBadgeCheck, HiOutlineGlobeAlt } from 'react-icons/hi';
...
export default function NosotrosPage() {
  ...
  const titulo = config?.quienes_somos_titulo || 'Quienes Somos';
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Contenido */}
      <div className="max-w-[900px] mx-auto px-5 sm:px-10 py-12">
        {cargando ? (
          <div className="flex justify-center py-16">
            <div className="w-8 h-8 border-3 border-gray-300 border-t-[#c0392b] rounded-full animate-spin" />
          </div>
        ) : !contenido && !mision && !vision ? (
          <p className="text-center text-[#4a4a4a] text-lg">Proximamente mas informacion sobre nosotros.</p>
        ) : (
          <>
            {contenido && (
              <p className="text-[16px] text-[#4a4a4a] leading-relaxed whitespace-pre-line mb-10">
                {contenido}
              </p>
            )}
            ...
            {(mision || vision) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {mision && (
                  <div className="bg-[#fcfcfc] rounded-[12px] border border-[#dcdcdc] p-6 shadow-[0_4px_15px_rgba(0,0,0,0.06)]">
                    <div className="flex items-center gap-2.5 mb-4">
                      <div className="w-10 h-10 bg-red-50 rounded-lg flex items-center justify-center">
                        <HiOutlineBadgeCheck className="w-6 h-6 text-[#c0392b]" />
                      </div>
                      <h2 className="font-condensed font-black text-[22px] text-[#1a1a1a] uppercase">Mision</h2>
                    </div>
                    <p className="text-[14px] text-[#4a4a4a] leading-relaxed whitespace-pre-line">{mision}</p>
                  </div>
                )}
                {vision && (
                  <div className="bg-[#fcfcfc] rounded-[12px] border border-[#dcdcdc] p-6 shadow-[0_4px_15px_rgba(0,0,0,0.06)]">
                    <div className="flex items-center gap-2.5 mb-4">
                      ...
                      <h2 className="font-condensed font-black text-[22px] text-[#1a1a1a] uppercase">Vision</h2>
                    </div>
                    <p className="text-[14px] text-[#4a4a4a] leading-relaxed whitespace-pre-line">{vision}</p>
                  </div>
                )}
              </div>
            )}
